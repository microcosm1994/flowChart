      ...
      <el-card class="box-card" shadow="hover">
        <div slot="header" class="clearfix header-box">
          <el-row>
              ...
              <div class="search-box">
                <el-input placeholder="请输入搜索内容" v-model="searchContent" class="input-with-select" size="mini" @keyup.native.13="search">
                  <el-select v-model="searchSelect" slot="prepend" placeholder="请选择">
                    ...
                    <el-option label="报表" value="user"></el-option>
                    <el-option label="报表" value="report"></el-option>
                    <el-option label="属性" value="var"></el-option>
            ...
            <el-col :span="4">
              <el-button type="primary" @click="create" size="mini" icon="el-icon-plus">新添故障树</el-button>
            </el-col>
          </el-row>
        </div>
        <el-table :data="tableData" border style="width: 100%" height="70vh">
          ...
          <el-table-column  prop="user"   label="报表"> </el-table-column>
          <el-table-column  prop="report"   label="报表"> </el-table-column>
          <el-table-column  prop="var"   label="属性"> </el-table-column>
        ...
        <el-dialog title="新添故障树" :visible="dialog" width="40%" :before-close="createClose">
          <el-form ref="dialogData" :model="dialogData" label-width="140px">
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item label="报表">
                  <el-input v-model="dialogData.report"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="24">
                <el-form-item label="属性">
                  <el-input v-model="dialogData.var"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 获取设备
      getDevice () {
        this.$http.post('/api/fta/valueLog/get', {n: 0, l: 1000}).then((response) => {
        ...
        if (id) {
          for (let i = 0; i < deviceData.length; i++) {
            if (id === deviceData[i].id) {
              ...
              this.$http.post('/api/fta/valueLog/get', {id: deviceData[i].$device_struct_ibfk_2, deleted: 0, n: 0, l: 100}).then((response) => {
                if (response.status === 200) {
          ...
          this.$http.post('/api/fta/valueLog/get', {deleted: 0, n: start, l: end}).then((response) => {
            if (response.status === 200) {
              this.tableData = response.data.d
              this.tableTotle = response.data.t
            }
      ...
      search () {
        ...
        this.$http.post('/api/fta/valueLog/get', data).then((response) => {
          if (response.status === 200) {
            this.tableData = response.data.d
            this.tableTotle = response.data.t
          }
      ...
      save () {
        let data = {
        	...
        	var: this.dialogData.var,
        	value: this.dialogData.value
        }
        // 新添
        if (this.saveType === 'create') {
          this.$http.post('/api/fta/valueLog/add', data).then((response) => {
            if (response.status === 200) {
        ...
        if (this.saveType === 'edit') {
          ...
          this.$http.post('/api/fta/valueLog/put', data).then((response) => {
            if (response.status === 200) {
