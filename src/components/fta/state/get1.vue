<template>
  <el-card class="box-card" shadow="hover">
          ...
          <div class="search-box">
            <el-input placeholder="请输入搜索内容" v-model="searchContent" class="input-with-select" size="mini" @keyup.native.13="search">
              <el-select v-model="searchSelect" slot="prepend" placeholder="请选择">
             	...
             	<el-option label="变量" value="objKey"></el-option>
    ...
    <el-table :data="tableData" border style="width: 100%" height="70vh">
      ...
      <el-table-column  prop="objKey"   label="变量"> </el-table-column>
     	...
     	<el-row>
          <el-col :span="24">
            <el-form-item label="评分">
              <el-input v-model="dialogData.score"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
     	<el-row>
...
<script>
  export default {
    ...
    methods: {
      // 获取设备
      getDevice () {
        this.$http.post('/api/fta/state/get', {n: 0, l: 1000}).then((response) => {
        ...
        if (id) {
          for (let i = 0; i < deviceData.length; i++) {
            if (id === deviceData[i].id) {
              ...
              this.$http.post('/api/fta/state/get', {id: deviceData[i].$device_struct_ibfk_2, deleted: 0, n: 0, l: 100}).then((response) => {
                if (response.status === 200) {
          ...
          this.$http.post('/api/fta/state/get', {deleted: 1, n: start, l: end}).then((response) => {
            if (response.status === 200) {
              this.tableData = response.data.d
              this.tableTotle = response.data.t
            }
      ...
      search () {
        ...
        this.$http.post('/api/fta/state/get', data).then((response) => {
          if (response.status === 200) {
            this.tableData = response.data.d
            this.tableTotle = response.data.t
          }
      ...
      save () {
        ...
        if (this.saveType === 'create') {
          this.$http.post('/api/fta/state/add', data).then((response) => {
            if (response.status === 200) {
        ...
        if (this.saveType === 'edit') {
          ...
          this.$http.post('/api/fta/state/put', data).then((response) => {
            if (response.status === 200) {
