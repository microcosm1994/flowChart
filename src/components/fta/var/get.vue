      ...
      <el-card class="box-card" shadow="hover">
        <div slot="header" class="clearfix header-box">
          <el-row>
              ...
              <div class="search-box">
                <el-input placeholder="请输入搜索内容" v-model="searchContent" class="input-with-select" size="mini" @keyup.native.13="search">
                  <el-select v-model="searchSelect" slot="prepend" placeholder="请选择">
                    ...
                    <el-option label="属性名" value="key"></el-option>
                    <el-option label="属性值" value="value"></el-option>
                    <el-option label="是否固定值" value="fix"></el-option>
            ...
            <el-col :span="4">
              <el-button type="primary" @click="create" size="mini" icon="el-icon-plus">新添中间时间表</el-button>
            </el-col>
          </el-row>
        </div>
        <el-table :data="tableData" border style="width: 100%" height="70vh">
          ...
          <el-table-column fixed="right"  label="操作" width="150px">
            <template slot-scope="scope">
                ...
                <el-button @click="jump(scope.row)" type="text" size="small">故障树</el-button>
                <!--<el-button type="text" size="small">-->
                  <!--<a href="#/fta/valueLog/get?var=" target="_blank">故障树</a>-->
                <!--</el-button>-->
              
              <el-button @click="edit(scope.row)" type="text" size="small">~{field.refFk.table.comment}</el-button>
        ...
        <el-dialog title="新添中间时间表" :visible="dialog" width="40%" :before-close="createClose">
          <el-form ref="dialogData" :model="dialogData" label-width="140px">
            <el-row>
              <el-col :span="24">
                <el-form-item label="用户">
                  <el-input v-model="dialogData.user"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="24">
                <el-form-item label="对象">
                  <el-input v-model="dialogData.obj"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item label="是否固定值">
                  <el-radio-group size="mini" v-model="dialogData.fix">
...
<script>
  ...
  export default {
    ...
    methods: {
      // 获取设备
      getDevice () {
        this.$http.post('/api/fta/var/get', {n: 0, l: 1000}).then((response) => {
        ...
        if (id) {
          for (let i = 0; i < deviceData.length; i++) {
            if (id === deviceData[i].id) {
              ...
              this.$http.post('/api/fta/var/get', {id: deviceData[i].$device_struct_ibfk_2, deleted: 0, n: 0, l: 100}).then((response) => {
                if (response.status === 200) {
          ...
          this.$http.post('/api/fta/var/get', {deleted: 0, n: start, l: end}).then((response) => {
            if (response.status === 200) {
              this.tableData = response.data.d
              this.tableTotle = response.data.t
            }
      ...
      search () {
        ...
        this.$http.post('/api/fta/var/get', data).then((response) => {
          if (response.status === 200) {
            this.tableData = response.data.d
            this.tableTotle = response.data.t
          }
      ...
      save () {
        ...
        if (this.saveType === 'edit') {
          ...
          this.$http.post('/api/fta/var/put', data).then((response) => {
            if (response.status === 200) {
      ...
      jump (row) {
        let id = row.id
        this.$router.push({
          path: '#/fta/~{field.refFk.table}/get',
          query: {
          ~{field.refFk.field}: id
          }
        })
      },
      // 删除
      remove (row) {
        this.$confirm('是否删除这个中间时间表？', '确认信息', {
