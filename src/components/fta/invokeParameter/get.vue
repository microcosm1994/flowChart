      ...
      <el-card class="box-card" shadow="hover">
        <div slot="header" class="clearfix header-box">
          <el-row>
              ...
              <div class="search-box">
                <el-input placeholder="请输入搜索内容" v-model="searchContent" class="input-with-select" size="mini" @keyup.native.13="search">
                  <el-select v-model="searchSelect" slot="prepend" placeholder="请选择">
                    ...
                    <el-option label="接口" value="invokeId"></el-option>
                    <el-option label="实参对象编号" value="objId"></el-option>
                    <el-option label="形参变量编号" value="parameterId"></el-option>
            ...
            <el-col :span="4">
              <el-button type="primary" @click="create" size="mini" icon="el-icon-plus">新添参数表</el-button>
            </el-col>
          </el-row>
        </div>
        <el-table :data="tableData" border style="width: 100%" height="70vh">
          ...
          <el-table-column  prop="invokeId"   label="接口"> </el-table-column>
          <el-table-column  prop="objId"   label="实参对象编号"> </el-table-column>
          <el-table-column  prop="parameterId"   label="形参变量编号"> </el-table-column>
        ...
        <el-dialog title="新添参数表" :visible="dialog" width="40%" :before-close="createClose">
          <el-form ref="dialogData" :model="dialogData" label-width="140px">
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item label="接口">
                  <el-input v-model="dialogData.invokeId"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="24">
                <el-form-item label="实参对象编号">
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item label="形参变量编号">
                  <el-input v-model="dialogData.parameterId"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
          </el-form>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 获取设备
      getDevice () {
        this.$http.post('/api/fta/invokeParameter/get', {n: 0, l: 1000}).then((response) => {
        ...
        if (id) {
          for (let i = 0; i < deviceData.length; i++) {
            if (id === deviceData[i].id) {
              ...
              this.$http.post('/api/fta/invokeParameter/get', {id: deviceData[i].$device_struct_ibfk_2, deleted: 0, n: 0, l: 100}).then((response) => {
                if (response.status === 200) {
          ...
          this.$http.post('/api/fta/invokeParameter/get', {deleted: 0, n: start, l: end}).then((response) => {
            if (response.status === 200) {
              this.tableData = response.data.d
              this.tableTotle = response.data.t
            }
      ...
      search () {
        ...
        this.$http.post('/api/fta/invokeParameter/get', data).then((response) => {
          if (response.status === 200) {
            this.tableData = response.data.d
            this.tableTotle = response.data.t
          }
      ...
      save () {
        let data = {
        	...
        	invokeId: this.dialogData.invokeId,
        	objId: this.dialogData.objId,
        	parameterId: this.dialogData.parameterId
        }
        // 新添
        if (this.saveType === 'create') {
          this.$http.post('/api/fta/invokeParameter/add', data).then((response) => {
            if (response.status === 200) {
        ...
        if (this.saveType === 'edit') {
          ...
          this.$http.post('/api/fta/invokeParameter/put', data).then((response) => {
            if (response.status === 200) {
