      ...
      <el-card class="box-card" shadow="hover">
        <div slot="header" class="clearfix header-box">
          <el-row>
              ...
              <div class="search-box">
                <el-input placeholder="请输入搜索内容" v-model="searchContent" class="input-with-select" size="mini" @keyup.native.13="search">
                  <el-select v-model="searchSelect" slot="prepend" placeholder="请选择">
                    ...
                    <el-option label="属性的对象类型" value="categoryId"></el-option>
                    ...
                    <el-option label="属性关键字" value="key"></el-option>
            ...
            <el-col :span="4">
              <el-button type="primary" @click="create" size="mini" icon="el-icon-plus">新添对象的属性（特征）</el-button>
            </el-col>
          </el-row>
        </div>
        <el-table :data="tableData" :header-row-style="{height: '70px'}" border style="width: 100%" height="70vh" :stripe="true">
          ...
          <el-table-column  prop="categoryId"   label="属性的对象类型"> </el-table-column>
          <el-table-column  prop="createTime"   label="创建时间点"> </el-table-column>
          <el-table-column  prop="updateTime"   label="更新时间点"> </el-table-column>
          <el-table-column  prop="key"   label="属性关键字"> </el-table-column>
          <el-table-column  prop="remark"   label="描述"> </el-table-column>
          <el-table-column fixed="right"  label="操作" width="150px">
            <template slot-scope="scope">
              
                <el-button @click="jump(scope.row, '/fta/categoryProperty/get')" type="text" size="small">分类的特征属性</el-button>
                <!--<el-button type="text" size="small">-->
                  <!--<a href="#/fta/categoryProperty/get?propertyId=" target="_blank">分类的特征属性</a>-->
                <!--</el-button>-->
        ...
        <el-dialog title="新添对象的属性（特征）" :visible="dialog" width="40%" :before-close="createClose">
...
<script>
  ...
  export default {
    ...
    methods: {
      // 获取设备
      getDevice () {
        this.$http.post('/api/fta/property/get', {n: 0, l: 1000}).then((response) => {
        ...
        if (id) {
          for (let i = 0; i < deviceData.length; i++) {
            if (id - 0 === deviceData[i].parent_id) {
              this.$http.post('/api/fta/property/get', {parent_id: id, deleted: 0, n: 0, l: 100}).then((response) => {
                if (response.status === 200) {
          ...
          this.$http.post('/api/fta/property/get', {deleted: 0, n: start, l: end}).then((response) => {
            if (response.status === 200) {
              this.tableData = response.data.d
              this.tableTotle = response.data.t
            }
      ...
      search () {
        ...
        this.$http.post('/api/fta/property/get', data).then((response) => {
          if (response.status === 200) {
            this.tableData = response.data.d
            this.tableTotle = response.data.t
          }
      ...
      save () {
        ...
        if (this.saveType === 'create') {
          this.$http.post('/api/fta/property/add', data).then((response) => {
            if (response.status === 200) {
        ...
        if (this.saveType === 'edit') {
          ...
          this.$http.post('/api/fta/property/put', data).then((response) => {
            if (response.status === 200) {
      ...
      // 删除
      remove (row) {
        this.$confirm('是否删除这个对象的属性（特征）？', '确认信息', {
