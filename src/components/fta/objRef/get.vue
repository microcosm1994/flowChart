    ...
    <el-tab-pane label="消息中心">
      <el-card class="box-card" shadow="hover">
              ...
              <div class="search-box">
                <el-input placeholder="请输入搜索内容" v-model="searchContent" class="input-with-select" size="mini" @keyup.native.13="search">
                  <el-select v-model="searchSelect" slot="prepend" placeholder="请选择">
                    ...
                    <el-option label="源" value="srcId"></el-option>
                    <el-option label="目标" value="dstId"></el-option>
                    <el-option label="属性" value="propertyId"></el-option>
        ...
        <el-table :data="tableData" :header-row-style="{height: '70px'}" border style="width: 100%" height="70vh" :stripe="true">
          ...
          <el-table-column  prop="srcId"   label="源"> </el-table-column>
          <el-table-column  prop="dstId"   label="目标"> </el-table-column>
          <el-table-column  prop="propertyId"   label="属性"> </el-table-column>
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item label="源">
                  <el-input v-model="dialogData.srcId"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="24">
                <el-form-item label="目标">
                  <el-input v-model="dialogData.dstId"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="24">
                <el-form-item label="属性">
                  <el-input v-model="dialogData.propertyId"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
          </el-form>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 获取设备
      getDevice () {
        this.$http.post('/api/fta/objRef/get', {n: 0, l: 1000}).then((response) => {
        ...
        if (id) {
          for (let i = 0; i < deviceData.length; i++) {
            if (id - 0 === deviceData[i].parent_id) {
              this.$http.post('/api/fta/objRef/get', {parent_id: id, deleted: 0, n: 0, l: 100}).then((response) => {
                if (response.status === 200) {
          ...
          this.$http.post('/api/fta/objRef/get', {deleted: 0, n: start, l: end}).then((response) => {
            if (response.status === 200) {
              this.tableData = response.data.d
              this.tableTotle = response.data.t
            }
      ...
      search () {
        ...
        this.$http.post('/api/fta/objRef/get', data).then((response) => {
          if (response.status === 200) {
            this.tableData = response.data.d
            this.tableTotle = response.data.t
          }
      ...
      save () {
        let data = {
        	...
        	srcId: this.dialogData.srcId,
        	dstId: this.dialogData.dstId,
        	propertyId: this.dialogData.propertyId
        }
        // 新添
        if (this.saveType === 'create') {
          this.$http.post('/api/fta/objRef/add', data).then((response) => {
            if (response.status === 200) {
        ...
        if (this.saveType === 'edit') {
          ...
          this.$http.post('/api/fta/objRef/put', data).then((response) => {
            if (response.status === 200) {
