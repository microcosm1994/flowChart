      ...
      <el-card class="box-card" shadow="hover">
        <div slot="header" class="clearfix header-box">
          <el-row>
            ...
            <el-col :span="4">
              <el-button type="primary" @click="create" size="mini" icon="el-icon-plus">新添模式特征类型</el-button>
            </el-col>
          </el-row>
        </div>
        <el-table :data="tableData" :header-row-style="{height: '70px'}" border style="width: 100%" height="70vh" :stripe="true">
          ...
          <el-table-column fixed="right"  label="操作" width="150px">
            <template slot-scope="scope">
                ...
                <el-button @click="jump(scope.row, '/fta/obj/get')" type="text" size="small">节点</el-button>
                <!--<el-button type="text" size="small">-->
                  <!--<a href="#/fta/obj/get?figure=" target="_blank">节点</a>-->
                <!--</el-button>-->
        ...
        <el-dialog title="新添模式特征类型" :visible="dialog" width="40%" :before-close="createClose">
...
<script>
  ...
  export default {
    ...
    methods: {
      // 获取设备
      getDevice () {
        this.$http.post('/api/fta/figureEnum/get', {n: 0, l: 1000}).then((response) => {
        ...
        if (id) {
          for (let i = 0; i < deviceData.length; i++) {
            if (id - 0 === deviceData[i].parent_id) {
              this.$http.post('/api/fta/figureEnum/get', {parent_id: id, deleted: 0, n: 0, l: 100}).then((response) => {
                if (response.status === 200) {
          ...
          this.$http.post('/api/fta/figureEnum/get', {deleted: 0, n: start, l: end}).then((response) => {
            if (response.status === 200) {
              this.tableData = response.data.d
              this.tableTotle = response.data.t
            }
      ...
      search () {
        ...
        this.$http.post('/api/fta/figureEnum/get', data).then((response) => {
          if (response.status === 200) {
            this.tableData = response.data.d
            this.tableTotle = response.data.t
          }
      ...
      save () {
        ...
        if (this.saveType === 'create') {
          this.$http.post('/api/fta/figureEnum/add', data).then((response) => {
            if (response.status === 200) {
        ...
        if (this.saveType === 'edit') {
          ...
          this.$http.post('/api/fta/figureEnum/put', data).then((response) => {
            if (response.status === 200) {
      ...
      // 删除
      remove (row) {
        this.$confirm('是否删除这个模式特征类型？', '确认信息', {
