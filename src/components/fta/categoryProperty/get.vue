      ...
      <el-card class="box-card" shadow="hover">
        <div slot="header" class="clearfix header-box">
          <el-row>
            ...
            <el-col :span="4">
              <el-button type="primary" @click="create" size="mini" icon="el-icon-plus">新添分类的特征属性</el-button>
            </el-col>
          </el-row>
        </div>
        ...
        <el-dialog title="新添分类的特征属性" :visible="dialog" width="40%" :before-close="createClose">
...
<script>
  ...
  export default {
    ...
    methods: {
      // 获取设备
      getDevice () {
        this.$http.post('/api/fta/categoryProperty/get', {n: 0, l: 1000}).then((response) => {
        ...
        if (id) {
          for (let i = 0; i < deviceData.length; i++) {
            if (id - 0 === deviceData[i].parent_id) {
              this.$http.post('/api/fta/categoryProperty/get', {parent_id: id, deleted: 0, n: 0, l: 100}).then((response) => {
                if (response.status === 200) {
          ...
          this.$http.post('/api/fta/categoryProperty/get', {deleted: 0, n: start, l: end}).then((response) => {
            if (response.status === 200) {
              this.tableData = response.data.d
              this.tableTotle = response.data.t
            }
      ...
      search () {
        ...
        this.$http.post('/api/fta/categoryProperty/get', data).then((response) => {
          if (response.status === 200) {
            this.tableData = response.data.d
            this.tableTotle = response.data.t
          }
      ...
      save () {
        ...
        if (this.saveType === 'create') {
          this.$http.post('/api/fta/categoryProperty/add', data).then((response) => {
            if (response.status === 200) {
        ...
        if (this.saveType === 'edit') {
          ...
          this.$http.post('/api/fta/categoryProperty/put', data).then((response) => {
            if (response.status === 200) {
      ...
      // 删除
      remove (row) {
        this.$confirm('是否删除这个分类的特征属性？', '确认信息', {
